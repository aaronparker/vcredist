Function Import-VcCmApp {
    ...
    Begin {        
        ...
        Set-Location -Path (Get-ValidPath $Path)
        ...
        If ( Test-Path $CMPath ) {
            ...
            If ($PSCmdlet.ShouldProcess("$($Path) to $($CMPath)", "Copy")) {
                Robocopy.exe *.exe $(Get-ValidPath $Path) $CMPath /S /XJ /R:1 /W:1 /NP /NJH /NJS /NFL /NDL
            }
        }
        Else {
    ...
    Process {
        ForEach ($Vc in $filteredVcList) {
            ...
            If ($PSCmdlet.ShouldProcess("$($Vc.Name) in $CMPath", "Import ConfigMgr app")) {
                ...
                Write-Verbose "Setting location to $(Get-ValidPath $Path)"
                Set-Location -Path (Get-ValidPath $Path)

...
If ($env:APPVEYOR_REPO_BRANCH -ne 'master') {
    ...
    # Test whether the manifest has the latest 2019 version
    $Version = . $projectRoot\ci\Update-Manifest.ps1
    If ($Null -ne $Version) {
        ...
        Try {
            ...
            git add --all
            git status
            git commit -s -m "Manifest update: $Version"
            Invoke-Process -FilePath "git" -ArgumentList "push origin $env:APPVEYOR_REPO_BRANCH"
            Write-Host "Manifest with $Version pushed to GitHub." -ForegroundColor Cyan
        }
        Catch {
            # Sad panda; it broke
